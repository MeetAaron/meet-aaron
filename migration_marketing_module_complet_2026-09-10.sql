...
-- ---------------------------------------------------------------------------
-- AJOUT DU 10/09/2026 — lien de desabonnement OPTIONNEL, DESACTIVE par defaut
-- (demande d'Alex : « desactive par defaut, j'y tiens »). Fusionne ici pour
-- qu'un seul passage suffise, puisque la migration d'origine n'avait jamais
-- ete jouee. Voir migration_campaign_unsubscribe_optionnel_2026-09-10.sql.
--
--   false (defaut) -> aucun pied de page ; l'email ressemble a un message
--                     ecrit a la main ;
--   true           -> pied de page avec le lien, dans la langue du commercial.
--
-- Le mecanisme de desabonnement reste entier dans les deux cas : la route de
-- suivi fonctionne toujours, et un prospect marketing_opt_out n'est jamais
-- reintegre a un envoi.
-- ---------------------------------------------------------------------------
...
comment on column public.marketing_campaigns.include_unsubscribe is
  'true = ajouter le pied de page de desabonnement aux emails de cette campagne. Defaut false (decision Alex 10/09/2026).';

...
const AARON_LABEL_NAME = '\u{1F916} Géré par Aaron';

// Le commercial voit ses échanges dans SA propre boîte Gmail (Aaron envoie/lit
// en son nom). Pour qu'il sache, en un coup d'oeil dans sa boîte, quels fils il
// ne doit PAS traiter lui-même, on pose un label Gmail dédié sur chaque fil
// qu'Aaron gère (voir AARON_LABEL_NAME ci-dessus pour le texte exact affiché).
// Créé une seule fois par compte, puis réutilisé (Gmail renvoie une erreur si
// on tente de recréer un label existant — on gère ça en listant d'abord plutôt
// qu'en se fiant à un cache qui pourrait devenir périmé si le commercial
// supprime le label lui-même).
...
// Pose le label Aaron (AARON_LABEL_NAME) sur un fil Gmail entier (donc sur
// tous les messages du fil, passés et à venir tant qu'ils y restent rattachés).
// Échec silencieux : un souci de label ne doit jamais empêcher l'envoi/la
// lecture d'un email, c'est purement un repère visuel pour le commercial.

...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    await sendEmailForUser(prospect.assigned_user_id, prospect.email, prospect.devis_subject, `${prospect.devis_body}${recapText}`);
  } catch (err: any) {
  ...
  const update: Record<string, any> = { devis_sent_at: sentAt };
  ...
  if (conversation) {
    await supabaseAdmin.from('messages').insert({
      ...
      body: `${prospect.devis_body}${recapText}`,
    });
  }
